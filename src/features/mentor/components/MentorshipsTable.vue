      ...
      <table class="w-full text-left border-collapse text-sm">
        <thead>
          <tr class="border-b border-slate-800 bg-slate-900/50 text-slate-400 font-mono text-xs uppercase tracking-wider">
            <th class="p-4 px-5 font-semibold">Team name / Applicant</th>
            <th class="p-4 px-5 font-semibold">Program</th>
            <th class="p-4 px-5 font-semibold">Appointment date</th>
            <th class="p-4 px-5 font-semibold">Status</th>
            <th class="p-4 px-5 font-semibold text-right">Actions</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-800/60 text-slate-300">
          <tr v-for="m in mentorships" :key="m.id" class="hover:bg-slate-800/20 transition group">
            <td class="p-4 px-5 font-medium text-white">
              {{ m.application?.team?.name || 'Without name (Individual)' }}
            </td>
            <td class="p-4 px-5 text-slate-400 font-mono text-xs uppercase tracking-wider">
              {{ m.application?.program_type ? 'Program ' + m.application.program_type.toUpperCase() : 'N/A' }}
            </td>
            <td class="p-4 px-5 text-slate-400 font-mono text-xs">
              {{ m.assigned_at ? new Date(m.assigned_at).toLocaleDateString('uk-UA') : '—' }}
            </td>
            <td class="p-4 px-5">
              <span 
