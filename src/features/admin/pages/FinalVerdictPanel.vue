<script setup lang="ts">
  ...
  async function submitFinalVerdict() {
    ...
    try {
      ...
      success.value = t('admin.finalVerdictPanel.messages.successSync', { status: finalStatus.value.toUpperCase() })
      setTimeout(() => router.push('/dashboard'), 3000)
    } catch (e) {
      error.value = t('admin.finalVerdictPanel.messages.submitFailed')
    } finally {
...
<template>
  <div class="bg-slate-950 min-h-screen text-slate-100 flex flex-col">
    <div class="border-b border-slate-800 bg-slate-900/60 px-6 py-4 flex items-center justify-between sticky top-0 z-30 backdrop-blur-md">
      <div class="flex items-center gap-4">
        <button @click="router.push('/dashboard')" class="text-slate-400 hover:text-white transition text-sm flex items-center gap-1.5">
          ← {{ t('admin.finalVerdictPanel.navigation.adminPanel') }}
        </button>
        <span class="text-slate-700">|</span>
        <h1 class="text-lg font-bold text-white flex items-center gap-2">
          {{ t('admin.finalVerdictPanel.title') }} <span class="font-mono text-blue-400">#{{ applicationId }}</span>
        </h1>
      </div>
      
      <div class="flex items-center gap-6">
        <div class="text-right hidden sm:block border-r border-slate-800 pr-6">
          <span class="text-xs text-slate-400 block">{{ t('admin.finalVerdictPanel.status.reviewsStatus') }}</span>
          <span class="text-sm font-mono font-semibold" :class="isCommitteeFinished ? 'text-blue-400' : 'text-slate-400'">
            {{ t('admin.finalVerdictPanel.status.done', { reviews: evaluatorsReviews.length, total: app?.total_evaluators_count ?? '-' }) }}
          </span>
        </div>

        <div class="text-right hidden sm:block">
          <span class="text-xs text-slate-400 block">{{ t('admin.finalVerdictPanel.status.consolidatedScore') }}</span>
          <span class="text-lg font-mono font-bold text-blue-400">
            {{ t('admin.finalVerdictPanel.status.scoreUnit', { score: globalAverageScore }) }}
          </span>
        </div>
        ...
        >
          {{ 
            submitting ? t('admin.finalVerdictPanel.buttons.saving') :
            isCommitteeFinished ? t('admin.finalVerdictPanel.buttons.publish') : 
            t('admin.finalVerdictPanel.buttons.incomplete') 
          }}
        </button>
      </div>
    </div>
    ...
    <div v-if="loading" class="flex-1 flex items-center justify-center text-slate-400 animate-pulse">
      {{ t('admin.finalVerdictPanel.loading') }}
    </div>
    ...
    <div v-else class="flex-1 grid grid-cols-1 lg:grid-cols-2 overflow-hidden">
      ...
      <div class="p-6 border-b lg:border-b-0 lg:border-r border-slate-800 lg:overflow-x-hidden max-h-[calc(100vh-69px)]">
        <div class="space-y-6">
          ...
          <!-- Documents -->
          <div>
            <h2 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">{{ t('admin.finalVerdictPanel.sections.attachments') }}</h2>
          ...
          <div>
            <h2 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">{{ t('admin.finalVerdictPanel.sections.peerSheets') }}</h2>
            <div class="space-y-4">
              <div v-for="review in evaluatorsReviews" :key="review.id" class="bg-slate-900/50 border border-slate-800/60 rounded-xl p-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mb-3">
                  <div v-for="s in review.scores" :key="s.id" class="bg-slate-950 p-2 rounded-lg border border-slate-900 flex flex-col justify-between">
                    ...
                    <p class="text-xs font-mono font-bold text-slate-300 mt-1">
                      {{ t('admin.finalVerdictPanel.peerSheetsFields.scoreUnit', { score: s.score, weight: s.weight_at_moment }) }}
                    </p>
                  </div>
                </div>
                ...
                <div class="bg-slate-950/60 rounded-lg p-2.5 border border-slate-900/60">
                  <p class="text-[11px] text-slate-400 italic leading-relaxed">
                    <span class="text-blue-400 not-italic font-bold text-[10px] block mb-0.5">{{ t('admin.finalVerdictPanel.peerSheetsFields.reviewerStatement') }}</span>
                    "{{ review.comment }}"
                  </p>
                </div>
      ...
      <div class="p-6 lg:overflow-y-auto max-h-[calc(100vh-69px)] bg-slate-900/10 flex flex-col justify-between">
        <div>
          ...
          <div class="bg-gradient-to-br from-blue-950/30 to-slate-900 border border-blue-500/20 rounded-xl p-5 mb-6 text-center">
            <span class="text-xs text-blue-400 uppercase tracking-widest font-mono font-semibold block mb-1">{{ t('admin.finalVerdictPanel.sections.averageIndex') }}</span>
            ...
            <p class="text-[11px] text-slate-400 mt-2 max-w-sm mx-auto">
              {{ t('admin.finalVerdictPanel.averageIndexDesc') }}
            </p>
          </div>

          <div class="bg-slate-900/40 border border-slate-800 p-5 rounded-xl mb-6">
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">{{ t('admin.finalVerdictPanel.sections.overrideStatus') }}</h3>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              ...
              >
                <span>{{ t('admin.finalVerdictPanel.statusOptions.approve.title') }}</span>
                <span class="text-[9px] font-normal text-slate-500">{{ t('admin.finalVerdictPanel.statusOptions.approve.desc') }}</span>
              </button>
              ...
              >
                ...
                <span class="text-[9px] font-normal text-slate-500">{{ t('admin.finalVerdictPanel.statusOptions.revision.desc') }}</span>
              </button>
              ...
              >
                <span>{{ t('admin.finalVerdictPanel.statusOptions.reject.title') }}</span>
                <span class="text-[9px] font-normal text-slate-500">{{ t('admin.finalVerdictPanel.statusOptions.reject.desc') }}</span>
              </button>
          ...
          <div class="bg-slate-900/40 border border-slate-800 p-5 rounded-xl mb-6">
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">{{ t('admin.finalVerdictPanel.sections.resolution') }}</h3>
            <p class="text-[11px] text-slate-500 mb-3">
              {{ t('admin.finalVerdictPanel.resolutionDesc') }}
            </p>
            <textarea 
              v-model="internalNotes" 
              rows="5" 
              :placeholder="t('admin.finalVerdictPanel.resolutionPlaceholder')" 
