...
onMounted(async () => {
  ...
  try {
    ...
    console.error(err)
    error.value = t('tasks.details.loading')
  } finally {
    loading.value = false
  }
})
...
const downloadDocument = async (doc: AttachedDocument) => {
  ...
  try {
    ...
    console.error('Error handling download file:', err)
  } finally {
...
const handleApply = () => {
  if (!task.value?.call_id) {
    useConfirm({
      title: t('tasks.details.errorTitle'),
      message: t('tasks.details.errorNoCall'),
      confirmText: t('tasks.details.btnConfirm'),
      danger: false,
    })
    return
  }
...
<template>
  <div class="px-6 md:px-20 py-10 text-gray-300 min-h-screen bg-slate-950 relative overflow-hidden">
    ...
    <button @click="goBack" class="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition mb-8 group">
      <span class="group-hover:-translate-x-1 transition-transform">←</span> {{ t('tasks.details.backToCatalog') }}
    </button>
    ...
    <div v-else-if="task" class="grid grid-cols-1 lg:grid-cols-3 gap-10 max-w-7xl mx-auto">
      ...
      <div class="lg:col-span-2 space-y-8">
        <div>
          <div class="flex items-center gap-3 mb-3">
            <span class="bg-blue-600/10 text-blue-400 text-xs font-semibold px-2.5 py-1 rounded border border-blue-900/60 uppercase">
              {{ t('tasks.details.programBTask') }}
            </span>
            <span class="text-gray-500 text-xs">{{ t('tasks.details.id', { id: task.id }) }}</span>
          </div>
        ...
        <div v-if="task.project_goal" class="bg-slate-900/40 border border-slate-900 p-6 rounded-2xl">
          <h2 class="text-lg font-bold text-white mb-3 flex items-center gap-2">
            <span class="w-1.5 h-4 bg-blue-500 rounded-sm"></span> {{ t('tasks.details.projectGoal') }}
          </h2>
        ...
        <div v-if="task.expected_outcome" class="bg-slate-900/40 border border-slate-900 p-6 rounded-2xl">
          <h2 class="text-lg font-bold text-white mb-3 flex items-center gap-2">
            <span class="w-1.5 h-4 bg-blue-500 rounded-sm"></span> {{ t('tasks.details.expectedOutcome') }}
          </h2>
        ...
        <div v-if="task.detailed_technical_description" class="space-y-3">
          <h2 class="text-xl font-bold text-white">{{ t('tasks.details.detailedSpecs') }}</h2>
        ...
        <div class="bg-slate-900/20 border border-slate-900 p-6 rounded-2xl space-y-4">
          <h2 class="text-lg font-bold text-white flex items-center gap-2">
            <span class="w-1.5 h-4 bg-blue-500 rounded-sm"></span> {{ t('tasks.details.attachmentsTitle') }}
          </h2>
            ...
            >
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  ...
                  <span class="text-[10px] text-gray-500 uppercase tracking-wider block">{{ t('tasks.details.attachedFileLabel') }}</span>
                </div>
              </div>
              ...
              >
                {{ downloadLoadingId === doc.id ? t('tasks.details.btnDownloading') : t('tasks.details.btnDownload') }}
              </button>
            </div>
          </div>

          <div v-else class="text-sm text-slate-500 italic p-2">
            {{ t('tasks.details.noDocuments') }}
          </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div v-if="task.architecture_requirements" class="border border-slate-900 p-5 rounded-xl bg-slate-900/20">
            <h3 class="text-sm font-bold text-blue-400 uppercase tracking-wider mb-2">{{ t('tasks.details.architectureReqs') }}</h3>
          ...
          <div v-if="task.integrations_apis" class="border border-slate-900 p-5 rounded-xl bg-slate-900/20">
            <h3 class="text-sm font-bold text-blue-400 uppercase tracking-wider mb-2">{{ t('tasks.details.apiIntegrations') }}</h3>
            <p class="text-gray-400 text-sm whitespace-pre-line">{{ task.integrations_apis }}</p>
          </div>
        </div>

        <div v-if="task.milestones" class="space-y-3">
          <h2 class="text-lg font-bold text-white">{{ t('tasks.details.milestones') }}</h2>
      ...
      <div class="space-y-6">
        ...
        <div class="border border-blue-900/40 bg-slate-950 p-6 rounded-2xl">
          <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">{{ t('tasks.details.projectOwner') }}</h3>
          <div class="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <span class="text-xs text-gray-500">{{ t('tasks.details.partnerTag') }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="border border-slate-900 bg-slate-900/30 p-6 rounded-2xl space-y-4">
          <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-slate-900 pb-2">
            {{ t('tasks.details.reqsAndRestrictions') }}
          </h3>

          <div class="flex justify-between items-center text-sm">
            <span class="text-gray-500">{{ t('tasks.details.budgetLabel') }}</span>
            <span class="font-bold text-green-400 text-base">
              {{ task.budget ? `€${task.budget}` : t('tasks.details.notSpecified') }}
            </span>
          </div>

          <div class="flex justify-between items-center text-sm">
            <span class="text-gray-500">{{ t('tasks.details.targetTeam') }}</span>
            <span class="text-white font-medium">
              {{ t('tasks.details.people', { min: task.call?.min_team_size || 3, max: task.call?.max_team_size || '∞' }) }}
            </span>
          </div>

          <div v-if="task.platforms" class="flex justify-between items-start text-sm">
            <span class="text-gray-500 shrink-0">{{ t('tasks.details.platforms') }}</span>
            <span class="text-white font-medium text-right">{{ task.platforms }}</span>
          </div>

          <div class="flex justify-between items-center text-sm">
            <span class="text-gray-500">{{ t('tasks.details.expectedDuration') }}</span>
            <span class="text-white font-medium">{{ task.expected_duration || t('tasks.details.durationDefault') }}</span>
          </div>
        ...
        <div v-if="task.required_technologies?.length || task.required_skills?.length" class="border border-slate-900 bg-slate-900/10 p-6 rounded-2xl space-y-5">
          <div v-if="task.required_technologies?.length">
            <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{{ t('tasks.form.labels.technologies') }}</h4>
          ...
          <div v-if="task.required_skills?.length">
            <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{{ t('tasks.form.labels.skills') }}</h4>
            <div class="flex flex-wrap gap-1.5">
        ...
        <div class="pt-4">
          ...
          >
            {{ t('tasks.details.btnSubmitApp') }}
          </button>
          
          <div v-else class="text-center p-3 bg-slate-900 rounded-xl border border-blue-900/20">
            <span class="text-xs text-gray-500 block mb-1">{{ t('tasks.details.statusLabel') }}</span>
