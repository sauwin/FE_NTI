...
async function submit() {
  errors.value = ''
  if (!name.value.trim()) {
    errors.value = t('student.teamForm.errNameRequired')
    return
  }
  ...
  try {
    ...
    errors.value = e?.response?.data?.message ?? t('student.teamForm.errCreateFailed')
  } finally {
...
<template>
  <div class="mb-6 border border-slate-800 rounded-xl p-5 bg-slate-950/40">
    <div class="flex items-center justify-between mb-5">
      <div>
        <h4 class="text-base font-bold text-white">{{ t('student.teamForm.title') }}</h4>
        <p class="text-sm text-slate-500 mt-1">{{ t('student.teamForm.description') }}</p>
      </div>
      ...
      >
        {{ t('student.common.close') }}
      </button>
    </div>

    <div v-if="errors" class="p-3 rounded-lg text-sm mb-4 border bg-red-900/20 border-red-800 text-red-400 font-mono">
      {{ t('student.common.systemError', { error: errors }) }}
    </div>

    <div class="grid grid-cols-1 gap-4">
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-mono uppercase text-slate-500">{{ t('student.teamForm.labelName') }}</label>
        <input
          v-model="name"
          type="text"
          :placeholder="t('student.teamForm.placeholderName')"
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-mono uppercase text-slate-500">{{ t('student.teamForm.labelDesc') }}</label>
        <textarea
          v-model="description"
          rows="3"
          :placeholder="t('student.teamForm.placeholderDesc')"
      ...
      <div class="flex items-center gap-2 pt-1">
        ...
        >
          {{ submitting ? t('student.teamForm.creating') : t('student.teamForm.btnCreate') }}
        </button>
